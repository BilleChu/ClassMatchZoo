...
def build_network(cnn_block_count):
    ...
    # CNV blocks
    filter_size = word_vector_dimension * 2
    x_layer = content_word_position_embedding_sequences
    print( 'conv filter size %d, block count %d' % (filter_size, cnn_block_count) )
    
    for i in range(0, cnn_block_count):
        conv_layer_a = Convolution1D( filter_size, kernel_size, activation='linear', padding='same',name='conv_block_conv_a_%d' % i )( x_layer )
        conv_layer_b = Convolution1D( filter_size, kernel_size, activation='sigmoid', padding='same',name='conv_block_conv_b_%d' % i )( x_layer )

        glu_layer = Multiply( name='glu_layer_%d' % i )( [conv_layer_a, conv_layer_b] )
        print( 'glu layer shape:' + str(glu_layer.shape) )
        conv_block_output_layer = Dense( word_vector_dimension, activation='elu', name='conv_block_attention_layer_%d' % i )( glu_layer )
        print( 'conv block output layer shape:' + str(conv_block_output_layer.shape) )   
        x_layer = Add( name='residual_layer_%d' % i )([ x_layer, conv_block_output_layer ])
        print( 'residual layer shape:' + str(x_layer.shape) )
    
    print( 'cnn_concat_layer.shape=%s' % str(x_layer.shape) )
